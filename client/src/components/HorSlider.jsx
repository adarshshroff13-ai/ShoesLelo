    ...
    return (
        ...
        <div>
            <div className="xs:m-2 xs:text-xs md:text-sm xs:w-[180px] xs:h-[250px] md:w-[250px] md:h-[300px] flex justify-between flex-col border-1 hover:border-black p-2 relative">
                ...
                {/* Product image */}
                <div className="xs:h-2/3 sm:h-1/2 md:h-2/3 w-full xs:text-sm md:text-base flex items-center justify-center overflow-hidden bg-gray-50">
                    <Link to={`/product-details/${product._id}`} className="w-full h-full flex items-center justify-center">
                        <img src={img} alt={title} className="w-full h-full object-contain hover:scale-105 transition-transform duration-300" />
                    </Link>
                </div>
